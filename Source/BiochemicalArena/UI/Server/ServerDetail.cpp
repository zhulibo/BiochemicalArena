...
UWidget* UServerDetail::NativeGetDesiredFocusTarget() const
{
	if (Lobby && Connect)
	{
		...
		if (Lobby->CurLobby->GetStringAttr(LOBBY_SESSIONID) == FString(""))
		{
			return ReadyButton;
		}
...
void UServerDetail::SetLobbyUIButton()
{
	if (Lobby && Connect)
	{
		...
		else
		{
			...
			JoinServerButton->SetIsEnabled(Lobby->CurLobby->GetStringAttr(LOBBY_SESSIONID) != FString(""));
...
void UServerDetail::RefreshPlayerList()
{
	...
	for (auto Member : Lobby->CurLobby->Members)
	{
		...
		if (PlayerLineButton)
		{
			...
			{
				PlayerLineButton->Status->SetText(FText::FromString(UServiceManager::GetLobby()->CurLobby->GetBoolAttr(LOBBY_MEMBER_BISREADY) ? "Ready" : ""));
			}
...
void UServerDetail::BackToServer()
{
	if (MenuController == nullptr) MenuController = Cast<AMenuController>(GetOwningPlayer());
	if (MenuController)
	{
